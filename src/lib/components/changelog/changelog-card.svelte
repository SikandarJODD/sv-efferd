...
>
	<div class="md:pt-0.5 sticky top-6 self-start">
		<ChangelogDateBadge date={entry.date} />
	</div>

	<div class="min-w-0">
		<h2 class="text-xl font-semibold tracking-tight text-foreground sm:text-xl">{entry.title}</h2>

		{#if entry.description}
			<p class="mt-4 max-w-2xl text-base leading-7 text-muted-foreground sm:text-base sm:leading-8">
				{entry.description}
			</p>
		{/if}
