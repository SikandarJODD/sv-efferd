...
<a class={cn('flex items-center gap-x-2', className)} {href} {...props}>
	<div
		class={cn(
			'rounded-lg flex aspect-square size-9 items-center justify-center border bg-card shadow-xs outline outline-offset-2 outline-border/60 [&_svg]:size-4 [&_svg]:text-foreground'
		)}
	>
		<IconComponent />
	</div>
